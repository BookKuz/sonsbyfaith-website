    ...
    <main className="min-h-screen bg-[#f7f2ea] text-stone-900">
      ...
      <section id="resources" className="mx-auto max-w-7xl px-5 py-16">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {filtered.map((item, index) => <ResourceCard key={item.title} item={item} index={index} />)}
        </div>
      </section>
      ...
      <section id="submit" className="mx-auto max-w-7xl px-5 py-16">
        <div className="grid gap-8 rounded-[2rem] border border-stone-200 bg-white p-8 shadow-sm lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <h2 className="mt-2 font-serif text-4xl font-bold text-stone-950">Upload new devotionals, sermons, poems, and essays.</h2>
            <p className="mt-5 leading-7 text-stone-600">This front-end mockup includes the publishing experience. In a real deployment, this would connect to a CMS like Sanity, WordPress, Supabase, or Payload so you can log in and publish without touching code.</p>
          </div>
      ...
      <footer className="border-t border-stone-200 px-5 py-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 md:flex-row md:items-center md:justify-between">
          ...
          <form className="flex w-full max-w-md gap-2">
            ...
            <button className="rounded-full bg-stone-950 px-5 py-3 text-white"><Mail className="h-4 w-4" /></button>
          </form>
        </div>
      </footer>
    </main>
